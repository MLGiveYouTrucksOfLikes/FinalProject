...
def grad_tree_adaboost(n_estimators=100, learning_rate=.1, saveModel = False):
    ...
    for train_index, test_index in kf:
        ...
        clf = GradientBoostingClassifier(n_estimators=n_estimators, learning_rate=learning_rate, \
                verbose= 1, max_depth=4, min_samples_leaf=9, max_features=0.3, subsample=0.7)
    ...
    if saveModel or bestscore >= .871:
        joblib.dump(bestmodel,'model/Grad_Tree_Adaboost_class_n'+str(n_estimators)+'_rate'+str(learning_rate)+'_fold'+str(fold)+'.pkl')
    return bestscore, bestmodel

if __name__ == '__main__':
    #do_grid_search()
    grad_tree_adaboost(5000, .01, True)
