...
'max_features': 0.3, 'learning_rate': 0.01, 'verbose': 1, 'max_depth': 4, 'min_samples_leaf': 9
'''
...
def dnn_tanh(x, y):
    layers = skflow.ops.dnn(x, [100, 200, 100], tf.tanh)
    return skflow.models.logistic_regression(layers, y)
def dnn(saveModel = False):
    # Argument to change
    ...
    for train_index, test_index in kf:
        ...
        testScore = metrics.accuracy_score(Y_cross_predict, Y_cross_test)
        print 'Eval',i,' = ', 1.0 - testScore
        print 'Train Score = ', metrics.accuracy_score(clf.predict(X_cross_train), Y_cross_train)
